...
// check if password is correct.
    ...
    public List<PricingInfoWithEntitlement> getAllPricingInfoWithFilter(String side,int entitlement){
		List<PricingInfoWithEntitlement> prices = new ArrayList<>();
		//Query query = em.createQuery("Select p.firm,p.price,p.product,p.qty,p.side,p.timeUpdated,p.tradeStatus from PricingInfoWithEntitlement p where p.product Like '%"+ search + "%' and p.entitlement >= "+entitlement,PricingInfoWithEntitlement.class);
		Query query = em.createQuery("Select p from PricingInfoWithEntitlement p where p.side = '"+ side + "' and p.entitlement >= "+entitlement,PricingInfoWithEntitlement.class);
		prices = query.getResultList();
    	System.out.println(prices);
		return prices;
    }
    ...
    public List<PricingInfoWithEntitlement> getAllPricingInfoWithDateFilter(Date date1,Date date2,int entitlement){
		List<PricingInfoWithEntitlement> prices = new ArrayList<>();
		Query query = em.createQuery("Select p from PricingInfoWithEntitlement p where p.TimeUpdated > '"+ date1 + "' and p.TimeUpdated < '"+ date2 + "' and p.entitlement >= "+entitlement,PricingInfoWithEntitlement.class);
		prices = query.getResultList();
    	System.out.println(prices);
		return prices;
    }
